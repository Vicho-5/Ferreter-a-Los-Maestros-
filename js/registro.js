...
const inputConfirmarContrasena = document.querySelector(
...
const errorConfirmarContrasena = document.querySelector(
...
const btnMostrarContrasena = document.querySelector(
    "#btn-mostrar-contrasena-registro",
);

const btnMostrarConfirmacion = document.querySelector(
...
function configurarFechaNacimiento() {
    ...
    inputFechaNacimiento.max = formatearFecha(hoy);

    inputFechaNacimiento.min = formatearFecha(fechaMinima);
}
...
function validarRun() {
    ...
    if (run.includes(".") || run.includes("-")) {
        mostrarError(inputRun, errorRun, "Ingresa el RUN sin puntos ni guion.");
    ...
    if (!formatoRun.test(run)) {
        mostrarError(inputRun, errorRun, "Ingresa un RUN válido.");
...
function validarConfirmarContrasena() {
    ...
    const confirmacion = inputConfirmarContrasena.value;
    ...
    limpiarError(inputConfirmarContrasena, errorConfirmarContrasena);
...
function validarTipoUsuario() {
    ...
    limpiarError(selectTipoUsuario, errorTipoUsuario);
...
selectRegion.addEventListener("change", function () {
    ...
    limpiarEstado(selectComuna, errorComuna);
});

// MOSTRAR / OCULTAR CONTRASEÑA

function cambiarVisibilidadContrasena(input, boton) {
...
btnMostrarContrasena.addEventListener("click", function () {
    cambiarVisibilidadContrasena(inputContrasena, btnMostrarContrasena);
});
...
inputRun.addEventListener("input", function () {
    if (inputRun.value.trim() === "") {
        limpiarEstado(inputRun, errorRun);
...
inputNombre.addEventListener("input", function () {
    if (inputNombre.value.trim() === "") {
        limpiarEstado(inputNombre, errorNombre);
...
inputApellidos.addEventListener("input", function () {
    if (inputApellidos.value.trim() === "") {
        limpiarEstado(inputApellidos, errorApellidos);
...
inputCorreo.addEventListener("input", function () {
    if (inputCorreo.value.trim() === "") {
        limpiarEstado(inputCorreo, errorCorreo);
...
inputContrasena.addEventListener("input", function () {
    if (inputContrasena.value === "") {
        limpiarEstado(inputContrasena, errorContrasena);
    ...
    if (inputConfirmarContrasena.value !== "") {
...
inputConfirmarContrasena.addEventListener("input", function () {
    if (inputConfirmarContrasena.value === "") {
        limpiarEstado(inputConfirmarContrasena, errorConfirmarContrasena);
...
inputNombre.addEventListener("animationstart", function (evento) {
    if (evento.animationName === "autofillDetectadoRegistro") {
        setTimeout(function () {
            validarNombre();
        }, 50);
    }
...
inputApellidos.addEventListener("animationstart", function (evento) {
    if (evento.animationName === "autofillDetectadoRegistro") {
        setTimeout(function () {
            validarApellidos();
        }, 50);
...
inputCorreo.addEventListener("animationstart", function (evento) {
    if (evento.animationName === "autofillDetectadoRegistro") {
        setTimeout(function () {
            validarCorreo();
        }, 50);
...
inputContrasena.addEventListener("animationstart", function (evento) {
    if (evento.animationName === "autofillDetectadoRegistro") {
        setTimeout(function () {
            validarContrasena();
        }, 50);
...
inputConfirmarContrasena.addEventListener("animationstart", function (evento) {
    if (evento.animationName === "autofillDetectadoRegistro") {
        setTimeout(function () {
            validarConfirmarContrasena();
        }, 50);
...
inputApellidos.addEventListener("blur", function () {
    inputApellidos.value = inputApellidos.value.trim();
});

inputCorreo.addEventListener("blur", function () {
    inputCorreo.value = inputCorreo.value.trim().toLowerCase();
});

inputDireccion.addEventListener("blur", function () {
    inputDireccion.value = inputDireccion.value.trim();
});
...
formulario.addEventListener("submit", function (evento) {
    ...
    if (
        ...
        const primerCampoConError = formulario.querySelector(".is-invalid");
    ...
    const correoExiste = usuarios.some(function (usuario) {
        return usuario.correo && usuario.correo.toLowerCase() === correo;
    });
    ...
    const runExiste = usuarios.some(function (usuario) {
        return usuario.run && usuario.run.toUpperCase() === run;
    });
    ...
    const nuevoUsuario = {
        ...
        region: regiones[indiceRegion].nombre,
    ...
    const modalRegistroExitoso = new bootstrap.Modal(
        document.querySelector("#modal-registro-exitoso"),
        {
            backdrop: "static",
            keyboard: false,
        },
